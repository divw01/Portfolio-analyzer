...
def holdings_cache_path(etf_symbol: str, asof_date: Optional[str] = None) -> Path:
    """Return the cache path for a given ETF and as-of date.

    If asof_date is provided, we store a daily snapshot under:
      data/etf_holdings/<ETF>/holdings_<YYYY-MM-DD>.parquet

    If asof_date is None, callers usually want "latest available"; see
    latest_cached_etf_holdings_date() + load_cached_etf_holdings().
    """
    ensure_dirs()
...
def latest_cached_etf_holdings_date(etf_symbol: str) -> Optional[str]:
    """Return the most recent cached YYYY-MM-DD date for an ETF, or None."""
    ensure_dirs()
    etf = symbol_norm(etf_symbol)
    folder = ETF_HOLDINGS_DIR / etf
    ...
    for p in folder.glob("holdings_*.parquet"):
        m = re.match(r"holdings_(\d{4}-\d{2}-\d{2})\.parquet$", p.name)
        if m:
...
def load_cached_etf_holdings(etf_symbol: str, asof_date: Optional[str] = None) -> Optional[pd.DataFrame]:
    """Load cached ETF holdings.

    Behavior:
      - If asof_date is provided: load that exact day's snapshot (if present).
      - If asof_date is None: load the latest cached snapshot for that ETF (if any).
    """
    if asof_date is None:
        latest = latest_cached_etf_holdings_date(etf_symbol)
        if latest is None:
            return None
        path = holdings_cache_path(etf_symbol, latest)
    else:
